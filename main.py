...
open(ANALYZED_AUTHOR_TARGET, 'w').write(
    "Author,AvgScore(/100),ScoredCount,FilledStud1,Score1,FilledStud2,Score2,FilledStud3,Score3,FilledStud4,Score4,FilledStud5,Score5,FilledStud6,Score6\n")
...
for id in tqdm(SPREADSHEET_IDS):
    response = ""
    afternoon = False
    try:
        response = service.spreadsheets().values().get(
    ...
    for r in response['values'][1:]:
        ...
        if (afternoon == True) and (submitDateTime.hour != 12):
            submitDateTime = submitDateTime.replace(
                hour=submitDateTime.hour + 12)
